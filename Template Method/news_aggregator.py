import urllib2  # Ta make http requests ro RSS and Atom feeds
...
class AbstractNewsParser(object):
    ...
    def __init__(self):
        # Prohibit creating class instances
...
class YahooParser(AbstractNewsParser):
    ...
    def get_url(self):
        return 'http://news.yahoo.com/res/'
    ...
    def parse_content(self, raw_content):
        ...
        for node in dom.getElementsByTagName('item'):
            ...
            try:
                parsed_item['title'] = node.getElementByTagName('title')[0].childNodes[0].nodeValue
            except IndexError:
                parsed_item['title'] = None

            try:
                parsed_item['content'] = node.getElementByTagName('description')[0].childNodes[0].nodeValue
            except IndexError:
                parsed_item['content'] = None

            try:
                parsed_item['link'] = node.getElementByTagName('link')[0].childNodes[0].nodeValue
            except IndexError:
                parsed_item['link'] = None

            try:
                parsed_item['id'] = node.getElementByTagName('guid')[0].childNodes[0].nodeValue
            except IndexError:
                parsed_item['id'] = None

            try:
                parsed_item['published'] = node.getElementByTagName('pubDate')[0].childNodes[0].nodeValue
            except IndexError:
